...
export default function ChatMessages(props) {
    let exportedHTML;
    let chatEvent = props;
    let thisParticipant = {id:1, name:'srting', avatar:'url', inSession:true, onStage:true};
    ...
    if (chatEvent.type === 'join'){
        ...
        exportedHTML = `<p className="notification">${thisParticipant.name} gave a thumbs up</p><span id='thumbsup' className="tui-icon icon-thumbsup"></span>`;
    }
    else if (chatEvent.type === 'thumbs-down'){
        exportedHTML = `<p className="notification">${thisParticipant.name} gave a thumbs down</p><span id='thumbsdown' className="tui-icon icon-thumbsdown"></span>`;
    }
    else if (chatEvent.type === 'raise-hand'){
        exportedHTML = `<p className="notification">${thisParticipant.name} raised their hand</p><span id='raisedHand' className="tui-icon icon-raisedHand"></span>`;
    }
    else if (chatEvent.type === 'clap') {
        exportedHTML = `<p className="notification">${thisParticipant.name} clapped</p><span id='clap' className="tui-icon icon-clap"></span>`;
    }
        ...
        exportedHTML = `<img class="chatAvatar" src=${thisParticipant.avatar}><p class="chatPrefix">${thisParticipant.name} ${thisTime}</p><p class="chatMessage">${chatEvent.message}</p>`;
    }
